 ...
 *
 * Responsibilities:
 *   1. Dispatch finalizeAccount with the collected basicInfo.
...
const TAConfirmationScreen: React.FC = () => {
  ...
  useEffect(() => {
    if (didFinalize.current || !createdAccount?.identifier || !basicInfo) return;
    ...
    dispatch(
      tradingAccountActions.finalizeAccount({
        ...
        accountName: basicInfo.businessName?.trim() || `${basicInfo.firstName} ${basicInfo.lastName}`.trim(),
        shortBio: basicInfo.shortBio,
        contactPhone: basicInfo.contactPhone,
        fullAddress: basicInfo.fullAddress,
        ...(basicInfo.profileImageBase64
          ? {profileImageBase64: basicInfo.profileImageBase64}
          : {}),
      }),
    );
