...
var mask = require('../../helpers/native/mask.cjs');
...
const KitsInputMask = ({ ref, ...rawProps }) => {
  ...
  const { value: displayValue, emitChange } = useFormInputController_native.useFormInputController({
    value,
    defaultValue,
    onChange
  });
  const handleChangeText = (raw) => {
    ...
    if (mask$1) {
      const res = mask.formatWithMask(raw, mask$1);
      masked = res.masked;
