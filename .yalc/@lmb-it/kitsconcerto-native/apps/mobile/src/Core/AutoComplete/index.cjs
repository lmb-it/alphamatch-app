...
function splitInputStyle(combined) {
  if (!combined) return {};
  const wrapper = {};
  const field = {};
  for (const [k, v] of Object.entries(combined)) {
    if (TEXT_STYLE_KEYS.has(k)) field[k] = v;
    else wrapper[k] = v;
  }
  return {
    wrapperOverrides: Object.keys(wrapper).length ? wrapper : void 0,
    fieldOverrides: Object.keys(field).length ? field : void 0
  };
}
function AutoComplete({
  ...
  const { wrapperOverrides, fieldOverrides } = React.useMemo(
    () => splitInputStyle(inputStyle),
    [inputStyle]
  );
  ...
  const resolvedBorderRadius = wrapperOverrides?.borderRadius ?? styles.inputWrapper.borderRadius;
        ...
        children: [
          ...
          /* @__PURE__ */ jsxRuntime.jsx(
            reactNative.TextInput,
            {
              ...
              onFocus: handleFocus,
              onBlur: handleBlur,
              style: [styles.inputField, { color: colors.text }, fieldOverrides]
            }
          ),
      ...
      reactNative.View,
      {
        style: [
          styles.panel,
          {
            ...
            borderRadius: Math.min(resolvedBorderRadius, 16)
          },
          panelStyle
        ],
        children: [
