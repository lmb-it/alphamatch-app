...
import { useState, useRef, useCallback, useImperativeHandle, useEffect, useMemo, createContext, useContext, Fragment } from 'react';
...
import Form from '../../form-builder/Form.js';
...
const DataView = (props) => {
  ...
  const [serverSideRequest, setServerSideRequest] = useState(requestDefaultParams);
  const [layout, setLayout] = useState(defaultMode ?? "grid");
  const listEnded = useRef(false);
  const searchedStarted = useRef(false);
  ...
  const fixData = useCallback((data2) => {
    return [...data2 || []].map((d) => {
      const row = {};
  ...
  const setState = (setFn, current, updated) => {
    if (!isEqual(current, updated)) setFn(updated);
  };
  ...
  const renderItem = ({ item, index }) => /* @__PURE__ */ jsx(Fragment, { children: layout === "list" ? listItem(item, index) : gridItem(item, index) }, item[keyColumn]);
  const header = () => /* @__PURE__ */ jsxs(Flex, { w: "full", flexDirection: "column", gap: 8, children: [
    !hideModeButtons && /* @__PURE__ */ jsxs(Flex, { flexDirection: "row", justifyContent: "flex-end", gap: 8, children: [
      /* @__PURE__ */ jsx(
        Button,
        {
          size: "sm",
    ...
    filtersFormElements && /* @__PURE__ */ jsxs(Flex, { gap: 8, w: "full", flexDirection: "row", children: [
      /* @__PURE__ */ jsx(Flex, { flex: 1, children: /* @__PURE__ */ jsx(
        Form,
        {
          ref: filterFormRef,
  ...
  return /* @__PURE__ */ jsx(
    DataViewContext.Provider,
    {
      ...
      children: /* @__PURE__ */ jsxs(Flex, { flexDirection: "column", flex: 1, children: [
        isHeader && header(),
        dataList.length === 0 && !isLoading ? /* @__PURE__ */ jsx(Flex, { p: 16, alignItems: "center", justifyContent: "center", children: /* @__PURE__ */ jsx(Text, { children: text?.emptyMessage }) }) : /* @__PURE__ */ jsx(
          FlatList,
          {
            data: dataList,
            renderItem,
        ...
        isLoading && /* @__PURE__ */ jsx(Flex, { w: "full", justifyContent: "center", alignItems: "center", p: 8, children: /* @__PURE__ */ jsx(ActivityIndicator, { size: "small" }) })
      ] })
    }
  );
};
...
export { DataViewContext, DataView as default, useDataView };
