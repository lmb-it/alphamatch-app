...
import { Switch } from '../../../../../apps/mobile/src/ui/switch/index.js';
...
const KitsInputSwitch = ({ ref, ...rawProps }) => {
  const { mergedProps: props, themeStyle } = useComponentDefaults("Switch", rawProps);
  const {
    id,
    label,
    helperText,
  ...
  const [internal, setInternal] = useState(incoming ?? false);
  useEffect(() => {
    if (isControlled) {
  ...
  const { nativeProps, cssProps } = useSeparator({ ...rest, ...localProps });
  return /* @__PURE__ */ jsx(
    KitsContainer,
    {
      id,
