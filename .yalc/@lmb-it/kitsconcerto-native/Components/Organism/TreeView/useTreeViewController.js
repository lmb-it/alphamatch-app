...
import { markAncestors, buildTree } from './utils.js';
...
const platformPrompt = (title, defaultValue) => {
  ...
  return new Promise((resolve) => {
    if (Platform.OS === "ios") {
      Alert.prompt(title, void 0, [
        { text: "Cancel", onPress: () => resolve(null), style: "cancel" },
        { text: "OK", onPress: (text) => resolve(text || null) }
      ], "plain-text", defaultValue);
    } else {
      Alert.alert(title, `Confirm to add "${defaultValue || "New Item"}"`, [
        { text: "Cancel", onPress: () => resolve(null), style: "cancel" },
...
const useTreeViewController = (props) => {
  ...
  const newNodes = useRef(/* @__PURE__ */ new Map());
  const newItems = useRef(/* @__PURE__ */ new Map());
  const editedNodes = useRef(/* @__PURE__ */ new Map());
  const editedItems = useRef(/* @__PURE__ */ new Map());
  const deletedNodes = useRef(/* @__PURE__ */ new Map());
  const deletedItems = useRef(/* @__PURE__ */ new Map());
  const nodesCategoriesChecked = useRef([]);
  const nodesItemsChecked = useRef([]);
  const nodesCategoriesUnChecked = useRef([]);
  const nodesItemsUnChecked = useRef([]);
  const setNodesMapping = (newMap) => {
    nodesMapping.current = newMap;
    setTreeValue(buildTree(nodesMapping.current, !!props.rules?.allowIndependentItem));
  };
    ...
    if (preselectList && nodesMapping.current.size > 0) {
      ...
      preselectList.forEach((item) => {
        ...
        if (node) {
          ...
          markAncestors(node.value, itemsMap, ps);
        }
      });
      setSelectedKeys(ps);
    }
  ...
  const addNode = useCallback(async (item = null, parentItemValue = null, isItem = false) => {
  ...
  const deleteNode = useCallback(async (item, deleted = null) => {
  ...
  const updateNode = useCallback(async (item, updated = false) => {
  ...
  const onAddNode = useCallback((parentItemValue = null) => {
  ...
  const onAddNodeItem = useCallback((parentItemValue) => {
    if (beforeAddItem) {
      const findParent = nodesMapping.current.get(parentItemValue?.toString() ?? "") ?? null;
  ...
  const onDeleteNodeWrapper = useCallback((itemValue) => {
  ...
  const onEditNodeWrapper = useCallback((itemValue) => {
    const find = nodesMapping.current.get(itemValue.toString());
    if (!find) return;
...
export { useTreeViewController };
//# sourceMappingURL=useTreeViewController.js.map
