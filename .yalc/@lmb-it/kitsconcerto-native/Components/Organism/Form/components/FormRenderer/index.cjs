...
const FormRenderer = ({
  ...
  const TEXT_TYPES = ["Text", "Email", "Number", "Password", "Phone"];
  const textFieldIds = React.useMemo(() => {
    return elements.filter((el) => TEXT_TYPES.includes(el.type)).map((el) => `${parentPath}${el.id}`);
  }, [elements, parentPath]);
  const keyboardNavCtx = useKeyboardNavigation.useKeyboardNavProvider();
  React.useEffect(() => {
  ...
  return /* @__PURE__ */ jsxRuntime.jsx(useKeyboardNavigation.KeyboardNavContext.Provider, { value: keyboardNavCtx, children: /* @__PURE__ */ jsxRuntime.jsx(
