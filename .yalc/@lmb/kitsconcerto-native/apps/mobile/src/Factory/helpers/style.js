...
import { breakpoints } from '../../../../../packages/types/src/Css/constants/breakpoints/index.js';
import { isThemeToken } from '../../../../../packages/types/src/Theme/resolvers.js';
import { CSS_VAR_MAP } from './cssVarMap.js';

let _activeColorMap = CSS_VAR_MAP;
...
function resolveCssVar(value) {
  ...
  return _activeColorMap[varName] ?? CSS_VAR_MAP[varName] ?? value;
}
...
const style = (props) => {
  ...
  const apply = (key, value) => {
    const mapped = allProperties[key];
    if (!mapped) return;
...
export { createStyleContext, getCurrentBreakpoint, resolveCssVar, resolveResponsiveValue, resolveThemeTokenForNative, setActiveColorMap, style, toNativeValue };
